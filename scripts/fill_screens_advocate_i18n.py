...
def main():
    ...
    screens_node = data.setdefault("screens", {}).setdefault("Advocate", {})

    updated = 0
    for file in sorted(TARGET.rglob("index.*")):
        if file.suffix.lower() not in {".ts", ".tsx", ".js", ".jsx"}:
            continue

        rel = file.relative_to(TARGET).with_suffix("")
        parts = list(rel.parts[:-1]) or ["index"]

        node = ensure_node(screens_node, parts)
    ...
    print(f"UPDATED_SCREENS_ADVOCATE_ENTRIES {updated}")
