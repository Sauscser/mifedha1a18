...
TARGETS = {
    "MFNdogo": ROOT / "components" / "MFNdogo",
    "MFKubwa": ROOT / "components" / "MFKubwa",
    "Advocate": ROOT / "components" / "Advocate",
}
        ...
        if "TouchableOpacity" in line or "Pressable" in line:
            for j in range(i, min(i + 12, len(lines))):
                ...
                if mt:
                    ...
                    if value and len(value) <= 100 and value not in buttons:
...
def main():
    ...
    updated = {}
    for namespace, target_dir in TARGETS.items():
        ns_node = comps.setdefault(namespace, {})
        updated_count = 0

        for file in sorted(target_dir.rglob("index.*")):
            if file.suffix.lower() not in {".ts", ".tsx", ".js", ".jsx"}:
                continue

            rel = file.relative_to(target_dir).with_suffix("")
            parts = list(rel.parts[:-1]) or ["index"]

            node = ensure_node(ns_node, parts)
        ...
        updated[namespace] = updated_count

    I18N.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print("UPDATED_COMPONENT_ENTRIES", json.dumps(updated, ensure_ascii=False))
