              ...
              const gtBiznaInfo2 = async () => {
                ...
                try {
                  ...
                  const CreateNewSMAc2 = async () => {
                    ...
                    const loanReqMsg7 = "NiSenti. " + busNames + ' business has requested ' + ' your Business entity to loan goods worth ' + itemPrys + '. Please go to your NiSenti' + ' app to view the loan details and thereafter' + ' grant me the request. Thank you.';
                    try {
                      const msgRes = await client.graphql({
                        query: createMessages,
                        variables: { input: { senderEmail: awsEmail, messageBody: loanReqMsg7 }}
                      });
                      if (msgRes?.data?.createMessages) {
                        await client.graphql({
                          query: sendNotification,
                          variables: { riderEmail: awsEmail, title: 'NiSenti: Credit Loan Request', body: loanReqMsg7 }
                        });
                      }
                    } catch (notifErr) {
                  ...
                  const gtAdv = async () => {
                    ...
                    try {
                      ...
                      const CreateNewSMAc = async () => {
                        ...
                        Alert.alert("Loan Request Successful");
                        const loanReqMsg8 = 'NiSenti. Greetings! ' + 'We ' + busNames + ', the loanee Business and ' + busName + ', the Loaning Business humbly' + ' request that you witness our loan contract on NiSenti app amounting to ' + itemPrys + ' repayable with ' + lnPrsntg + '% interest by the end of ' + rpymntPrd + ' days. Default penalty is ' + MmbaID + '. You can reach my loaner through ' + awsEmail + '. You can also reach us through ' + awsEmail2 + '. Thank you.';
                        try {
                          const msgRes = await client.graphql({
                            query: createMessages,
                            variables: { input: { senderEmail: phonecontact, messageBody: loanReqMsg8 }}
                          });
                          if (msgRes?.data?.createMessages) {
                            await client.graphql({
                              query: sendNotification,
                              variables: { riderEmail: phonecontact, title: 'NiSenti: Witness Loan Contract', body: loanReqMsg8 }
                            });
                          }
                        } catch (notifErr) {
                          console.log('Notification error:', notifErr);
                  ...
                  if (e) {
                    Alert.alert("Error! Update app or contact customer care");
                  }
                  console.error(e);
                }
                setIsLoading(false);
              ...
              <View style={styles.image}>
                <ScrollView>
                  ...
                  <View style={styles.sendLoanView}>
                    <TextInput placeholder='Business Phone' value={awsEmail} onChangeText={setAWSEmail} style={styles.sendLoanInput} editable={true}></TextInput>
                    <Text style={styles.sendLoanText}>Loaner Business Phone</Text>
                  </View>

                  <View style={styles.sendLoanView}>
                    <TextInput placeholder='Business Phone' value={awsEmail2} onChangeText={setAWSEmail2} style={styles.sendLoanInput} editable={true}></TextInput>
                  ...
                  <View style={styles.sendLoanView}>
                    <TextInput placeholder='Installment Frequency (Days' keyboardType='decimal-pad' value={InstFreq} onChangeText={setInstFreq} style={styles.sendLoanInput} editable={true}></TextInput>
