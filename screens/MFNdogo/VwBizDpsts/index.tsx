...
import NonLnRec from "../../../components/MyAc/VwDepositsMFN";
import { getAgent, getBizna, VwMFNFltDeductns, VwMyUsrDposits } from '../../../src/graphql/queries';
...
const FetchSMNonLnsSnt = () => {
  ...
  const fetchLoanees = async () => {
    ...
    try {
      ...
      const items = result.data.VwMFNFltDeductns.items || [];
      setAllRecords(items);
      setFilteredRecords(items);
      if (items.length < 1) {
        Alert.alert("No deposits");
      }
    } catch (e) {
  ...
  return <View style={styles.container}>
      ...
      <TextInput placeholder="Depositing entity Name. Even partially" value={buyerFilter} onChangeText={setBuyerFilter} style={styles.input} />
