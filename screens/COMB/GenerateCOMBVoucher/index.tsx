...
const SellerConsumablesVoucherScreen = () => {
  ...
  const [resolvedFunderEmail, setResolvedFunderEmail] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchSellerNat = async () => {
      try {
        ...
        const sellerBiz = bizRes?.data?.getBizna;
        const email = sellerBiz?.email;
        setSellerLatitude(sellerBiz?.latitude ?? null);
        setSellerLongitude(sellerBiz?.longitude ?? null);
        if (email) {
      ...
      if (parentData) {
        try {
          if (parentData.funderType === 'funderTypeBiz') {
            ...
            setResolvedFunderEmail(email || null);
            if (email) {
              const smRes: any = await client.graphql({ query: getSMAccount, variables: { awsemail: email } });
            ...
            const email = parentData.funderAccount;
            setResolvedFunderEmail(email || null);
  ...
  const handleGenerateVoucher = useCallback(async () => {
    ...
    try {
        ...
        await client.graphql({
          ...
          variables: {
            input: {
              ...
              sellerLatitude: sellerLatitude ?? 0,
              sellerLongitude: sellerLongitude ?? 0,
              buyerLatitude: 0,
              buyerLongitude: 0,
      ...
      if (resolvedFunderEmail && resolvedFunderEmail !== parent.consumerEmail) {
        await client.graphql({
          query: createMessages,
          variables: {
            input: {
              senderEmail: resolvedFunderEmail,
              messageBody: t.messageBody
            }
          }
        });
        await client.graphql({
          query: sendNotification,
          variables: {
            riderEmail: resolvedFunderEmail,
            title: t.notificationTitle,
            body: t.notificationBody
          }
        });
  ...
  const generateVoucherButtonText = (() => {
    const count = Object.keys(voucherItems).length;
    if (t.generateVoucher?.includes('{{count}}')) {
      return t.generateVoucher.replace('{{count}}', String(count));
    }
    return `${t.generateVoucher} — ${count} ${t.items}`;
  })();
        ...
        {Object.keys(voucherItems).length > 0 && <>
          ...
          marginTop: 10
            ...
            fontWeight: 'bold'
                ...
                {updating ? t.generating : generateVoucherButtonText}
              </Text>
            </Pressable>
          </>}
      </Animated.View>
