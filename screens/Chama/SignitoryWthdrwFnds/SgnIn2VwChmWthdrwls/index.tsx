...
import { listChamaMembers, VwMFNFltDeductns, VwMyUsrDposits, VwMyUsrWthdrwls } from '../../../../src/graphql/queries';
  ...
  const fetchDeposits = async (groupContact) => {
    ...
    try {
      const res: any = await client.graphql({
        ...
        variables: {
          withdrawerid: groupContact,
          sortDirection: 'DESC',
          limit: 100
        }
      });
      setDeposits(res.data?.VwMyUsrWthdrwls?.items || []);
    } catch (e) {
      console.error('fetchDeposits error:', e);
      setError(t.errorAccessDenied || 'Failed to load withdrawals');
    } finally {
    ...
    <View style={styles.image}>
      ...
      {depositsLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.center}>{t.noWithdrawals || 'No withdrawals found.'}</Text>}
        />
      )}
    </View>
  );
