...
import { createChamaDepositSync, createChamaDividendsSync, createChamaLoanSync, createChamaWithdrawalSync, updateGroup } from '../../../src/graphql/mutations';
...
const SMCvLnStts = (props: SMAccount & { onSyncComplete: () => void }) => {
  const {
    SMAc: { grpContact, grpName, signitoryContact, signitoryName, WithdrawalSync },
    onSyncComplete,
  } = props;
  ...
  const gtUsrDtls4AdminDtls = async () => {
    ...
    try {
      ...
      await client.graphql({
        query: updateGroup,
        variables: { input: { grpContact, WithdrawalSync: 0 } }
      });
      ...
      await client.graphql({
        ...
        variables: {
          input: {
            amount: Number(WithdrawalSync) || 0,
            GrpAc: grpContact,
            ...
            BankName: "Equity",
            BranchNu: BranchNu,
            transactionType: "WithdrawalSync",
  ...
  return (
    <View style={styles.pageContainer}>
      <View style={styles.card}>
        ...
        <Text style={styles.prodInfo}><Text style={styles.label}>Sync Amount:</Text> {formatAmountSync(Math.floor(WithdrawalSync), userCode, ratesMap)}</Text>
      </View>
