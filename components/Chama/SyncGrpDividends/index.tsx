...
const SMCvLnStts = (props: SMAccount & { onSyncComplete: () => void }) => {
  const {
    SMAc: { grpContact, grpName, signitoryContact, signitoryName, MemberDividendSync },
    onSyncComplete,
  } = props;
  ...
  const gtUsrDtls4AdminDtls = async () => {
    ...
    try {
      ...
      await client.graphql({
        query: updateGroup,
        variables: { input: { grpContact, MemberDividendSync: 0 } }
      });
      ...
      await client.graphql({
        ...
        variables: {
          input: {
            amount: Number(MemberDividendSync) || 0,
            GrpAc: grpContact,
  ...
  return (
    <View style={styles.pageContainer}>
      <View style={styles.card}>
        ...
        <Text style={styles.prodInfo}><Text style={styles.label}>Sync Amount:</Text> {formatAmountSync(Math.floor(MemberDividendSync), userCode, ratesMap)}</Text>
      </View>
